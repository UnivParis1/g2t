...
//						echo "sql (Statut seul) = $sql \n";
				...
				else
				{
							...
							foreach ($declarationliste as $declaration)
							{
								if (strcasecmp($declaration->statut(),"r")!=0)
								{
									...
									$sql = sprintf("UPDATE DECLARATIONTP SET STATUT='r' WHERE DECLARATIONID='%s'",
											$fonctions->my_real_escape_utf8($declaration->declarationTPid()));
									mysql_query($sql);
									$erreur_requete=mysql_error();
									if ($erreur_requete!="")
					...
					// -----------------------------------------------------
					elseif (($fonctions->formatdatedb($datedebut) < $fonctions->formatdatedb($affectation->datedebut()))
				   		...
				   		if ($numquotite == $denomquotite)
				   		{
				   			...
				   			if (!is_null($declarationliste))
				   			{
				   				foreach ($declarationliste as $declarationTP)
				   				{
				   					...
				   					{
				   						$sql = sprintf("UPDATE DECLARATIONTP SET DATEDEBUT='%s', DATEFIN='%s' WHERE DECLARATIONID='%s'",
				   								$fonctions->my_real_escape_utf8($fonctions->formatdatedb($datedebut)),
				   								$fonctions->my_real_escape_utf8($fonctions->formatdatedb($datefin)),
				   								$fonctions->my_real_escape_utf8($declarationTP->declarationTPid()));
				   						mysql_query($sql);
				   						$erreur_requete=mysql_error();
				   						if ($erreur_requete!="")
				   						{
