            ...
            if ($fonctions->formatdatedb($affectation->datedebut()) <= $fonctions->formatdatedb($date)) 
            {
                ...
                elseif (in_array($codeinterne, array(structure::MAIL_AGENT_ENVOI_RESP_COURANT,structure::MAIL_RESP_ENVOI_RESP_PARENT))) // On envoie le mail au responsable
                {
                    ...
                    if ($sihamresp->agentid() != $destinatairemail->agentid() and $sihamresp->agentid() != "" and !array_key_exists($sihamresp->agentid(),$listuserspeciaux))
                    {
                        ...
                        $deleguserid = $delegation->delegationuserid;
                    ...
                    echo "On ajoute le responsable (" . $destinatairemail->identitecomplete()  . ") dans les destinataires. \n";
...
Merci de contrôler le dossier RH du responsable ou le gestionnaire saisi dans G2T.<br>";
    ...
    foreach ($mail_resp as $agentid => $listedemande) 
    {
        ...
        $agentcron->sendmail($responsable, "Demandes de congés ou d'autorisations d'absence en attente", $corpsmail, null);
        unset($responsable);
    }
    foreach ($mail_gest as $agentid => $listedemande) 
    {
        ...
        $agentcron->sendmail($gestionnaire, "Demandes de congés ou d'autorisations d'absence en attente", $corpsmail, null);
        unset($gestionnaire);
    }
    unset($agentcron);
