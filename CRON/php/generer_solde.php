    ...
    while ($result = mysqli_fetch_row($query)) {
        ...
        if ($fonctions->formatdatedb($struct->datecloture()) >= $fonctions->formatdatedb(date("Ymd"))) {
            ...
            if (! is_null($tablisteagent)) {
                ...
                $cronmail->sendmail($gest, 'Récapitulatif des congés pour la structure ' . $struct->nomcourt(), "Veuillez trouver ci-joint le récapitulatif des congés pour la structure " . $struct->nomcourt() . " à la date du " . date("d/m/Y") . ".<br>", $filename, null, true);
            }
        }
    }
