...
<?php	
	...
	if (isset($_POST["date_fin"]))
	{
		...
		{
			$msg_erreur = $msg_erreur . "La date de fin n'est pas initialisée ou est incorrecte (JJ/MM/AAAA) !!! <br>";
			$datefausse = TRUE;
		}
	}
	else
	...
	if ($msg_erreur == "" and !$datefausse)
	{
		...
		{
			$msg_erreur = $msg_erreur . "Vous ne pouvez pas faire de déclaration en dehors de la période d'affectation <br>";
		}
	}
	...
	// On verifie qu'il y a autant de case à coché marqué que de jour de TP a saisir
	if ($nbredemiTP != "" and !$datefausse)
	{
		...
		// Si la case à cocher nocheckquotite n'est pas coché on vérifie la répartition de la quotité
		// <=> Si elle est coché on ne fait pas de test de répartition
		if ($nocheckquotite != 'yes')
		{
			if ($nbsemainepaire != $nbredemiTP or $nbsemaineimpaire != $nbredemiTP)
			{
				$msg_erreur = $msg_erreur . "Vous devez saisir $nbredemiTP demie(s) journée(s) pour les semaines paires et impaires <br>";
			}
		}
		else
			echo "<br>La fonction 'Pas de controle de la quotité' est activée... Aucun contrôle n'est réalisé.<br>";  
	}
