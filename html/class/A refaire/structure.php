...
class structure {
	...
	function load($structureid)
	{
		if (is_null($this->structureid))
		{
			...
			if (mysql_num_rows($query) == 0)
				echo "Structure->Load (HARP_STRUCTURE) : Structure $structureid non trouvé <br>";
			...
			if (mysql_num_rows($query) == 0)
				echo "Structure->Load (ARTT_STRUCTURE) : Structure $structureid non trouvé <br>";
			$result = mysql_fetch_row($query);
	...
	function type($type = null)
	{
		if (is_null($type))
		{
			if (is_null($this->type))
				echo "Structure->type : Le type de la structure n'est pas défini !!! <br>";
			else
				return $this->type;
		}
		else
	...
	function agentlist($sousstrucuture = null)
	{
		...
		while ($result = mysql_fetch_row($query))
		{
			...
			$agent->load("$result[0]");
			// On vérifie que l'agent est bien dans la structure courante... Sinon on le prend pas
