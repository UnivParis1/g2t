...
//	}
	...
	function enTP($date = null, $moment = null)
	{
		...
		elseif (is_null($this->tabtpspartiel)) {
			$errlog = "DeclarationTP->enTP : Le tableau des RTT n'est pas initialisé !!!";
			echo $errlog."<br/>";
			error_log(basename(__FILE__)." ".$this->fonctions->stripAccents($errlog));
		}
...
//			$this->id = mysql_query($sql,$this->dbconnect);
		...
		else
		{
			...
			{
//				echo "###############################<br>";
//				echo "###### WARNING !!!!! Il faut penser a supprimer les demandes qui ne sont plus dans la pÃ©riode de TP #######<br>";
//				echo "########################################<br>";
