    ...
    if (!is_null($cree_option))
    {
        ...
        {       // On vérifie au moment de traiter la demande d'option s'il n'y a pas de demande en cours (cas du F5 dans le navigateur ou du double onglet dans le navigateur)
            ...
            else
            {
                ...
                $constantename = 'OPTIONCET_MODELE';
                $pdfmodele = 'Formulaire_CET_Option.pdf';
                ...
                {
                    $erreur  = "La création du droit d'option dans eSignature a échoué ==> Pas de sauvegarde du droit d'option dans G2T.";
...
//    if (!is_null($esignature_delete))
    if (!is_null($esignatureid_delete) and trim($esignatureid_delete)<>'')
    {
        ...
        else
        {
            ...
            {
                $esignature = new esignature($dbcon);
                $return = $esignature->delete_signrequest($esignatureid_delete);
...
<?php     
        ...
        else
        {
            ...
            foreach ((array)$demandeliste as $demande)
            {
                ...
                if ((strcasecmp((string)$statut, demande::DEMANDE_ATTENTE) == 0) and (strcasecmp((string)$demande->type(),'cet')==0)) // Une demande de congés sur CET est en attente de validation ==> On ne peut pas saisir d'option sur CET
                {
